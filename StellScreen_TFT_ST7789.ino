...
SSD1306Wire display2(0x3d, SDA, SCL);   
...
String menu_action[5] = {"Select. object", "Scope to obj.", "Scope to center", "Center selected obj.", "--- BACK ---"};
...
String ref_telescope = "5";                                                 // reférence télescope dans Stellarium
String requete =""; String httpRequestData = ""; int httpResponseCode = 0; String serverPath = "";

int menu_value = 0, old_menu_value=1, strate_menu = 0, compteur= 0, menu_targets_value = 0, menu_action_value = 0;
...
http_get_j2000();delay(50);
 ...
 if (digitalRead(SCROLL)== false){       
       ...
       if (strate_menu==2) {menu_targets_value ++; if (menu_targets_value == 5) {menu_targets_value=0;}}  // mettre à jour en cas d'ajout/supp d'options
       if (strate_menu==3) {menu_action_value ++; if (menu_action_value == 5) {menu_action_value=0;} draw_menu_action();}
  }
 ...
 if (digitalRead(VALID)==false){
  ...
  if (strate_menu==0){tft.fillScreen(TFT_BLACK);  strate_menu=1; draw_menu_principal();}
  ...
  else if (strate_menu==3) {
    if      (menu_action_value == 0) {strate_menu=2;}   
    ...
    else if (menu_action_value == 4) {tft.fillScreen(TFT_BLACK); strate_menu=1; draw_menu_principal();}   // back
    } 
 }
